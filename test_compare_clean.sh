...
while IFS= read -r cmd || [ -n "$cmd" ]; do
    ...
    # Ejecutar en minishell y limpiar prompt, ANSI y eco del input
    minishell_output=$(echo "$cmd" | $MINISHELL 2>&1         | sed '/minishell.*\$>/d'         | sed 's/minishell.*\$>//g'         | sed '/^\s*$/d'         | grep -vFx "$cmd"         | sed 's/\x1b\[[0-9;]*m//g'         | cat -e)
