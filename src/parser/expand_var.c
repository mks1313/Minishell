...
/*   Updated: 2025/04/15 20:08:40 by mmarinov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void expand_variable(t_tkn *tokens, t_env *env, int l_e_s)
{
    ...
    while (current_token)
    {
        if (!current_token->single_quote)
        {
            ...
            // Eliminar comillas dobles si están al principio y al final
            ...
            // Eliminar comillas simples si están al principio y al final
            if (expanded[0] == '\'' && expanded[ft_strlen(expanded) - 1] == '\'')
            {
                char *tmp_expanded = ft_substr(expanded, 1, ft_strlen(expanded) - 2);
                free(expanded);
