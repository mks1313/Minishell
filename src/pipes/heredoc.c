...
/*   Updated: 2025/05/03 15:21:46 by mmarinov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    while (cmd)  // Iterar sobre los comandos
    {
        t_redir *r = cmd->redirs;  // Iterar sobre las redirecciones del comando
        while (r)
        {
            if (r->type == REDIR_HEREDOC)  // Si la redirección es un heredoc
                create_heredoc_file(r, shell);   // Llamar a la función para crear el archivo heredoc
            r = r->next;  // Avanzar a la siguiente redirección
        }
        cmd = cmd->next;  // Avanzar al siguiente comando
    }
}
